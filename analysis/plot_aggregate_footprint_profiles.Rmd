...
### NRF1 profiles in GM12878

We scanned NRF1 motif matches along the genome, then extended 100bp window on both sides of motif matches. 

We downloaded DNase-seq, ATAC-seq and NRF1 ChIP-seq data in GM12878 from ENCODE. 

```{r}
# sites <- readRDS('/project2/xinhe/kevinluo/footprint_clustering/processed_data/hg38/NRF1_MA0506.1_1e-5.candidate.sites.rds')

sites_chip_labels <- readRDS('/project2/xinhe/kevinluo/footprint_clustering/processed_data/hg38/NRF1.GM12878.sites.chip.labels.rds')

dnase_count_matrix <- readRDS('/project2/xinhe/kevinluo/footprint_clustering/processed_data/hg38/NRF1.GM12878.DNase.counts.mat.rds')
# combine counts on both strands
dnase_count_matrix <- dnase_count_matrix[,1:(ncol(dnase_count_matrix)/2)] + dnase_count_matrix[,(ncol(dnase_count_matrix)/2+1):ncol(dnase_count_matrix)]

atac_count_matrix <- readRDS('/project2/xinhe/kevinluo/footprint_clustering/processed_data/hg38/NRF1.GM12878.ATAC.counts.mat.rds')
# combine counts on both strands
atac_count_matrix <- atac_count_matrix[,1:(ncol(atac_count_matrix)/2)] + atac_count_matrix[,(ncol(atac_count_matrix)/2+1):ncol(atac_count_matrix)]
```
...
dnase_neg_profile <- colMeans(dnase_count_matrix[neg_idx, ], na.rm = TRUE)
plot_pos_neg_profiles(dnase_pos_profile, dnase_neg_profile, 
                      title = "Aggregate DNase-seq profiles around NRF1 motifs in GM12878")

atac_pos_profile <- colMeans(atac_count_matrix[pos_idx, ], na.rm = TRUE)
atac_neg_profile <- colMeans(atac_count_matrix[neg_idx, ], na.rm = TRUE)
plot_pos_neg_profiles(atac_pos_profile, atac_neg_profile, 
                      title = "Aggregate ATAC-seq profiles around NRF1 motifs in GM12878")
```
...
plot_data_matrix_heatmap(pwm, data.l, chip.df, rank, 
                         data_name = c("DNase-seq", "ATAC-seq"), 
                         chip_name = c("ChIP-seq\ncounts", "ChIP-seq\npeaks"), 
                         title = "NRF1 sites in GM12878",
                         zMax_data = c(1, 3), 
                         zMax_chip = c(200, 1))
```


### NRF1 profiles in K562

We scanned NRF1 motif matches along the genome, then extended 100bp window on both sides of motif matches. 

We downloaded DNase-seq, ATAC-seq and NRF1 ChIP-seq data in K562 from ENCODE. 

```{r}
# sites <- readRDS('/project2/xinhe/kevinluo/footprint_clustering/processed_data/hg38/NRF1_MA0506.1_1e-5.candidate.sites.rds')

sites_chip_labels <- readRDS('/project2/xinhe/kevinluo/footprint_clustering/processed_data/hg38/NRF1.K562.sites.chip.labels.rds')

dnase_count_matrix <- readRDS('/project2/xinhe/kevinluo/footprint_clustering/processed_data/hg38/NRF1.K562.DNase.counts.mat.rds')
# combine counts on both strands
dnase_count_matrix <- dnase_count_matrix[,1:(ncol(dnase_count_matrix)/2)] + dnase_count_matrix[,(ncol(dnase_count_matrix)/2+1):ncol(dnase_count_matrix)]

atac_count_matrix <- readRDS('/project2/xinhe/kevinluo/footprint_clustering/processed_data/hg38/NRF1.K562.ATAC.counts.mat.rds')
# combine counts on both strands
atac_count_matrix <- atac_count_matrix[,1:(ncol(atac_count_matrix)/2)] + atac_count_matrix[,(ncol(atac_count_matrix)/2+1):ncol(atac_count_matrix)]
```
...
dnase_neg_profile <- colMeans(dnase_count_matrix[neg_idx, ], na.rm = TRUE)
plot_pos_neg_profiles(dnase_pos_profile, dnase_neg_profile, 
                      title = "Aggregate DNase-seq profiles around NRF1 motifs in K562")

atac_pos_profile <- colMeans(atac_count_matrix[pos_idx, ], na.rm = TRUE)
atac_neg_profile <- colMeans(atac_count_matrix[neg_idx, ], na.rm = TRUE)
plot_pos_neg_profiles(atac_pos_profile, atac_neg_profile, 
                      title = "Aggregate ATAC-seq profiles around NRF1 motifs in K562")
```
